...
public class FlaskApiClient {
    ...
    public List<Integer> getSimilarRecipes(Long recipeId) {
        try {
            // 요청 본문 생성
            Map<String, Object> requestBody = Map.of("recipe_id", recipeId);
            ...
            // Flask 서버로 POST 요청 보내기
            ResponseEntity<List> response = restTemplate.postForEntity(flaskServerUrl + "/similar-recipes", requestEntity, List.class);
            ...
            System.err.println("Error calling Flask /similar-recipes: " + e.getMessage());
        }
